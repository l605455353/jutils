...
public class ZipUtils {
    ...
    public static void zipFile(String resourcePath, String targetPath) {
        ...
        if (!targetFile.exists())targetFile.mkdirs();//目的文件不存在，则新建
        String targetName = resourceFile.getName() + ZIP_FILE_SUFFIX;
        ZipOutputStream out = null;
        try {
            FileOutputStream outputStream = new FileOutputStream(targetPath+"//"+targetName);
            out = new ZipOutputStream(new BufferedOutputStream(outputStream));

            compressedFile(out, resourceFile, "");
